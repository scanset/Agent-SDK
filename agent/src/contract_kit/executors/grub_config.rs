...
pub struct GrubConfigExecutor {
    contract: CtnContract,
}

impl GrubConfigExecutor {
...
impl CtnExecutor for GrubConfigExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "grub_config validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "grub_config validation failed:\n  - {}",
