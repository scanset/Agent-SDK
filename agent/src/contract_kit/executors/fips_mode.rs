...
pub struct FipsModeExecutor {
    contract: CtnContract,
}

impl FipsModeExecutor {
...
impl CtnExecutor for FipsModeExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "fips_mode validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "fips_mode validation failed:\n  - {}",
