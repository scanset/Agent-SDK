...
pub struct OsReleaseExecutor {
    contract: CtnContract,
}

impl OsReleaseExecutor {
...
impl CtnExecutor for OsReleaseExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "os_release validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "os_release validation failed:\n  - {}",
